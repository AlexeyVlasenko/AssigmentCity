...
class CardComponent extends Component {
    ...
    getWeather = async () => {
      ...
      const temp = `${parseInt(weather.main.temp - 273.15, 10)}°`;
    ...
    renderDistance = ({ location }) => {
      ...
      if (location) {
        ...
        distance = getDistance([latitude, longitude], cityLocation);
      }

      return (
        <View style={styles.distanceContainer}>
          <Text style={Theme.textStyles.distance}>{location ? `${parseInt(distance, 10)} KM` : ''}</Text>
    ...
    render() {
      const {
        city, onLike, isLiked, navigation,
      } = this.props;
      const {
      ...
      return (
        <Surface style={styles.surfaceContainer}>
          <TouchableRipple
            onPress={() => navigation.navigate('City', { cityId: id })}
                  ...
                  <View style={styles.locationContainer}>
                    <View style={styles.rowContainer}>
                      ...
                      <View style={styles.infoTextContainer}>
                        <Text
                          style={Theme.textStyles.temperature}
                        >
                          {weatherLoading ? '' : `${weather.temp} ${weather.condition}`}
